...
{
    public partial class ReporteJornadaIncompleta : Form
    {
        public ReporteJornadaIncompleta()
        {
            InitializeComponent();
        }
        ConsultasSQL sql = new ConsultasSQL();

        private void ReporteJornadaIncompleta_Load(object sender, EventArgs e)
        {
            dataGridView1.DataSource = sql.ReporteJornadaIncompleta();
        }

        private void buscar_btn_Click(object sender, EventArgs e)
        {
            string fecha = filtro_dtp.Value.ToString("dd/MM/yyyy");
            dataGridView1.DataSource = sql.ObtenerReporteJornadaIncompletaBuscar(fecha);
        }
